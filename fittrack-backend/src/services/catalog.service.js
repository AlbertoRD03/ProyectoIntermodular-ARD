...
const seedIfEmpty = async () => {
  ...
  const [zonesCount, typesCount, exercisesCount] = await Promise.all([
    BodyZone.estimatedDocumentCount(),
    WorkoutType.estimatedDocumentCount(),
    ExerciseCatalog.estimatedDocumentCount()
  ]);

  if (zonesCount === 0) {
    await BodyZone.insertMany([
  ...
  if (exercisesCount === 0) {
    await ExerciseCatalog.insertMany([
